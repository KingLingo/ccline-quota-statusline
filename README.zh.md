...
三种方式任选其一。**方案 A / B 是让 AI 帮你一键接入，方案 C 是手动。**
...
在终端里跑（Codex 会自己完成 clone + 安装）：

```bash
codex "克隆 https://github.com/KingLingo/ccline-quota-statusline 并执行 node install.mjs，把 ccline 额度进度条接入 Claude Code 状态栏"
```

> 非交互场景可用 `codex exec "…同上…"`。注意：进度条本身是 **Claude Code 状态栏**的能力，Codex 自身没有等价的命令式状态栏；这里是让 Codex 这个 agent 帮你把工具装进 Claude Code。想在 Codex 会话旁边随手看额度，用下面的「独立查额度」。
...
输出就是那三条进度条（不含 ccline 行），适合在用 Codex 或任何终端时快速看一眼额度。若已 `npm install -g`（或 `npm link`），可直接用 `ccline-quota` 命令。`--help` 看用法。
...
默认按 环境变量 → `settings.local.json` → `settings.json` 的顺序读取 `ANTHROPIC_BASE_URL` / `ANTHROPIC_API_KEY`。想只给额度查询单独指定，用 `QUOTA_BASE_URL` / `QUOTA_API_KEY` 覆盖。
